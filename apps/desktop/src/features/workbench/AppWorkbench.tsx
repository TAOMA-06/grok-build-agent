...
import { DirtyWorktreeDialog } from "../shell/DirtyWorktreeDialog";
...
export function AppWorkbench() {
  ...
  }, [addBlock, bridge, ensureSession, sessionsQuery.data]);
  ...
  useEffect(() => {
    if (activeSessionId && appView === "home") {
      // keep home until user selects a thread; don't auto-jump
    }
  }, [activeSessionId, appView]);
  ...
  useEffect(() => {
    function onEscape(event: KeyboardEvent) {
      ...
      else if (taskPanelOpen && appView !== "thread") setTaskPanelOpen(false);
    }
    window.addEventListener("keydown", onEscape);
    return () => window.removeEventListener("keydown", onEscape);
  }, [commandOpen, inspectorOpen, taskPanelOpen, appView]);
  ...
  const showChat = appView === "home" || appView === "thread";
  const showInspector = showChat && inspectorOpen && Boolean(activeSession);

  const rootClass = [
    "wb-root",
    taskPanelOpen ? "panel-open" : "",
    ...
    <div className={rootClass}>
      ...
      <main className="wb-stage">
        ...
        {appView === "settings" && (
          <SettingsDialog
            ...
            onOpenChange={() => setAppView(activeSessionId ? "thread" : "home")}
            onReloadAgent={async () => {
      ...
      {showInspector && activeSession && (
        <div className="wb-inspector">
          <ContextDrawer session={activeSession} onClose={() => setInspectorOpen(false)} />
        </div>
      )}
            ...
            <div className="gb-command-actions">
              {filteredCommandActions.map((command) => (
                ...
                >
                  ...
                  <span>
                    {command.label}
                    {command.aliases.length ? <small>{command.aliases.join(" · ")}</small> : null}
                  </span>
                  <i>{command.available ? command.source : t.unavailable}</i>
                </button>
              ))}
            </div>
          </Dialog.Content>
