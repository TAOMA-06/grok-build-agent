...
Each first user instruction seeds the task focus shown in the Context drawer. Settings provide Economy and Balanced focus profiles: Economy repeats a bounded short anchor between less-frequent full contracts, while Balanced refreshes the full contract more often. Strict Privacy Shield is the default local outbound guardrail; it redacts detected text secrets and blocks high-risk attachment names before local persistence or dispatch. It does not replace upstream Grok/xAI account privacy settings; see [`../../PRIVACY.md`](../../PRIVACY.md).
