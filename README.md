...
- New installs default to **durable tasks** (Private Chat off) with **Orchestrator harness** on: task contracts, verification commands, and plan/explore/verify guidance are active by default.
- New tasks seed a structured **task contract** (goal, acceptance, inferred verify commands) and auto-run declared verifications after each turn when present.
...
- 新任务会种子化结构化**任务合同**（目标、验收、推断的验证命令），并在声明验证后自动跑检查。
