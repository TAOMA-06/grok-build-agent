...
## Latest compatibility · Grok Build CLI v0.2.106

Grok Build Desktop now tracks the latest Grok Build CLI workflow (0.2.104–0.2.106) with:

- Orchestrator harness guidance for Grok 4.5 defaults, `/summarize`|`/recap`, and in-place scheduled / background work
...
### 最新适配 · Grok Build CLI v0.2.106

Grok Build Desktop 已适配最新 Grok Build CLI 工作流（0.2.104–0.2.106），新增：

- Orchestrator harness 对齐 Grok 4.5 默认模型、`/summarize`|`/recap`，以及可原地更新的定时 / 后台任务约定
- 支持多行输入的桌面任务编辑器
...
- New installs default to **durable tasks** (Private Chat off) with **Orchestrator harness** on: task contracts, verification commands, orchestration rules, and session harness skills (Grok Build **0.2.106**) when the package path resolves.
...
- 新安装默认可恢复任务（Private Chat 关）并开启 **编排 Harness**：任务合同、验证命令与计划/探索/验证引导默认生效（对齐 Grok Build **0.2.106**）。
